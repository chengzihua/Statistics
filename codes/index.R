...
source("E:/Statistics-SAR/codes/myread.ENVI.R")
source("E:/Statistics-SAR/codes/imagematrix.R")
require(ggplot2)
require(reshape2)
require(ggthemes)
# 导入数据，只有一个HH波段的数据，数据比较大，群里百度网盘里有数据包，最后上传github时候记得拿出来别一起传上去
# 数据在这 https://pan.baidu.com/s/1yIOIsA1-tNR_KMyN1kqnSA
imagepath <- "E:/Statistics-SAR/data/"
...
example <- HH_Intensity[1500:1599,1000:1050]
write.table(example,"E:/Statistics-SAR/data/urban.dat")
save(file="E:/Statistics-SAR/data/urban.Rdata",example)
...
ggplot(data=data.frame(vexample), aes(x=HH)) +
  geom_histogram(aes(y = ..density..), bins=100, col="black", fill="white") +
  #stat_function(fun=LogLikelihoodLknown, args = list(rate=1/mean(dark_HH)), lwd=1, col="red") +
